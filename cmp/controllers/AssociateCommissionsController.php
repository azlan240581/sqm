 ...
 */
class AssociateCommissionsController extends Controller
{
    ...
    {		
        $searchModel = new UserCommissionsSearch(['userGroup'=>array(11)]);
		...
		else
		{
			foreach($logUserCommissions as $key=>$value)
			{
				if($value['status']==2)
				{
					...
					{
						$logUserCommissions[$key]['status'] = 3;
						$logClaimedElgibible = LogUserCommission::find()->where(array('user_eligible_commission_id'=>$value['user_eligible_commission_id'],'status'=>3))->asArray()->one();
					...
					elseif($userEligibleCommission->status==1)
					{
						...
						$logUserCommissions[$key]['status'] = 4;
						$logClaimedElgibible = LogUserCommission::find()->where(array('user_eligible_commission_id'=>$value['user_eligible_commission_id'],'status'=>4))->asArray()->one();
						$logUserCommissions[$key]['createdbyName'] = Yii::$app->AccessMod->getName($logClaimedElgibible['createdby']);
						$logUserCommissions[$key]['createdat'] = $logClaimedElgibible['createdat'];
					}
					else
