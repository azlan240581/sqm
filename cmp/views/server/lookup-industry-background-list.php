...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'lookup-industry-background-list', $request, '', '');
...
{
	$result = Yii::$app->AccessMod->getLookupData('lookup_industry_background');
	$data['data'] = $result;
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'lookup-industry-background-list', $request, json_encode($data), 1);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
