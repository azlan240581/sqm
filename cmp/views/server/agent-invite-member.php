...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'agent-invite-member', $request, '', '');
...
if(!strlen($data['error']))
{
	$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'agent-invite-member', $request, json_encode($data), $user['id']);
	if(isset($logapi['error']))
	$data['error'] = '0000-'.$logapi['error'];
}
