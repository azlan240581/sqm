<?php
/* 
lookup-property-product-type-list
*/
...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'lookup-property-product-type-list', $request, '', '');
...
{
	$result = Yii::$app->AccessMod->getLookupData('lookup_property_product_types');
	$data['data'] = $result;
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'lookup-property-product-type-list', $request, json_encode($data), 1);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
