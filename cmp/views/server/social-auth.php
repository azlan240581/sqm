...
//create Log API
$logapi = Yii::$app->AccessMod->LogAPI('', 'social-auth', $request, '', '');
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];

//validate
if(!strlen($data['error']) && (empty($_REQUEST['email'])))
$data['error'] = '0001-Incorrect email.';

//process
/* get user profiles */
if(!strlen($data['error']))
{
	$user = Yii::$app->AccessRule->socialAuth($_REQUEST['email'],true,isset($_REQUEST['device_token'])?$_REQUEST['device_token']:'',isset($_REQUEST['device_os'])?$_REQUEST['device_os']:'');
...
if(!strlen($data['error']))
{
	...
	if(!$commission)
	$data['error'] = '0006-'.Yii::$app->CommissionMod->errorMessage;
}
...
if(!strlen($data['error']))
{
	...
	if(!$userhistory)
	$data['error'] = '0007-'.Yii::$app->CatalogMod->errorMessage;
}
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'social-auth', $request, json_encode($data), $user['id']);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
