...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'member-get-agent-details', $request, '', '');
...
if(!strlen($data['error']))
{
	try
	{
		...
		throw new ErrorException("Agent ID is required.");
		else
...
if(!strlen($data['error']))
{
	$result = Yii::$app->MemberMod->memberGetAgentDetails($user['id'],$agent_id);
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'member-get-agent-details', $request, json_encode($data), $user['id']);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
