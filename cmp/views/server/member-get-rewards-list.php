...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'member-get-rewards-list', $request, '', '');
...
if(!strlen($data['error']))
{
	$result = Yii::$app->CatalogMod->getRewardsList(!empty($_REQUEST['category_id'])?$_REQUEST['category_id']:'',!empty($_REQUEST['reward_id'])?$_REQUEST['reward_id']:'');
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'member-get-rewards-list', $request, json_encode($data), $user['id']);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
