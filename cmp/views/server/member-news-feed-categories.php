...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'member-news-feed-categories', $request, '', '');
...
if(!strlen($data['error']))
{
	$result = Yii::$app->NewsFeedMod->getNewsFeedCategory();
...
if(!strlen($data['error']))
{
	$data['news_feed_categories'] = $result;
}

//update log api
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'member-news-feed-categories', $request, json_encode($data), $user['id']);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
