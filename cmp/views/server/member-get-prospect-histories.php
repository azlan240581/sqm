...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'member-get-prospect-histories', $request, '', '');
...
//validate
if(!strlen($data['error']) && empty($_REQUEST['prospect_id']))
$data['error'] = '0008-Invalid prospect id.';
else
{
	$data['prospect_id'] = $_REQUEST['prospect_id'];
	if(!Yii::$app->ProspectMod->validateProspectOwner($data['prospect_id'],array('member_id'=>$user['id'])))	
	$data['error'] = '0009-Invalid prospect id.';
}
...
if(!strlen($data['error']))
{
	...
	if(!($data['prospect_histories'] = $processProspect->getProspectHistories($data['prospect_id'])))
	$data['error'] = '0010-'.$processProspect->errorMessage;
}
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'member-get-prospect-histories', $request, json_encode($data), $user['id']);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
