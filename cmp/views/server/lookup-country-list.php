...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'lookup-country-list', $request, '', '');
...
{
	$result = Yii::$app->AccessMod->getLookupData('lookup_country','iso2,iso3');
	$data['data'] = $result;
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'lookup-country-list', $request, json_encode($data), 1);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
