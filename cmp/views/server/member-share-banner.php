...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'member-share-banner', $request, '', '');
...
if(!strlen($data['error']))
{
	try
	{
		...
		throw new ErrorException("Banner ID is required.");
		else
		{
			$inputs['action_name'] = 'SHARE_BANNER';
			$inputs['banner_id'] = $_REQUEST['banner_id'];
		}
	}
...
if(!strlen($data['error']))
{
	$result = Yii::$app->PointsMod->memberPointsActivities($user['id'],'BANNER_SHARE_SOCIAL_MEDIA',1,$inputs);
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'member-share-banner', $request, json_encode($data), $user['id']);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
