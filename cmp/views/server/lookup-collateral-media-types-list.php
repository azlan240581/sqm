...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'lookup-collateral-media-types-list', $request, '', '');
...
{
	$result = Yii::$app->AccessMod->getLookupData('lookup_collateral_media_types');
	$data['data'] = $result;
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'lookup-collateral-media-types-list', $request, json_encode($data), 1);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
