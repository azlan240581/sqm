...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'user-message', $request, '', '');
...
if(!strlen($data['error']))
{
	...
	else
	{
		...
		{
			if(!array_intersect(array(7,8,11), $groups))
			$data['error'] = '0006-Invalid group';
		}
	}
}
...
if(!strlen($data['error']))
{
	$result = Yii::$app->AccessMod->userMessage($user['id'],(isset($_REQUEST['message_id'])?$_REQUEST['message_id']:''));
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'user-message', $request, json_encode($data), $user['id']);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
