<?php
/* 
lookup-prospect-level-interest-list
*/
...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'lookup-prospect-level-interest-list', $request, '', '');
...
{
	$result = Yii::$app->AccessMod->getLookupData('lookup_prospect_level_interest','','id');
	$data['data'] = $result;
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'lookup-prospect-level-interest-list', $request, json_encode($data), 1);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
