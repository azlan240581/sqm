...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'lookup-project-products-list', $request, '', '');
...
{
	$result = Yii::$app->GeneralMod->getProjectProducts(!empty($_REQUEST['project_id'])?$_REQUEST['project_id']:'');
	$data['data'] = $result;
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'lookup-project-products-list', $request, json_encode($data), 1);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
