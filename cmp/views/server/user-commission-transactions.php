...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'user-commission-transactions', $request, '', '');
...
if(!strlen($data['error']))
{
	try
	{
		if(empty($_REQUEST['prospect_booking_id']))
		throw new ErrorException("Prospect Booking ID is required.");
		else
		$prospect_booking_id = $_REQUEST['prospect_booking_id'];
	}
...
if(!strlen($data['error']))
{
	$result = Yii::$app->CommissionMod->getLogUserCommissionTransaction($user['id'],$prospect_booking_id);
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'user-commission-transactions', $request, json_encode($data), $user['id']);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
