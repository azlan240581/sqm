...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'member-news-feeds', $request, '', '');
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'member-news-feeds', $request, json_encode($data), $user['id']);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
