...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'lookup-reward-categories-list', $request, '', '');
...
{
	$result = Yii::$app->AccessMod->getLookupData('lookup_reward_categories');
	$data['data'] = $result;
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'lookup-reward-categories-list', $request, json_encode($data), 1);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
