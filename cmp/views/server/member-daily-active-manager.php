...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'member-daily-active-manager', $request, '', '');
...
if(!strlen($data['error']))
{
	try
	{
		...
		throw new ErrorException("Action is required.");
		else
		{
			if(!in_array(strtolower($_REQUEST['action']),array('list','redeem')))
...
if(!strlen($data['error']))
{
	$result = Yii::$app->MemberMod->memberDailyActiveManager($user['id'],$action);
...
{
	$data['success'] = true;
	$data['daily_active_manager'] = $result;
}

//update log api
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'member-daily-active-manager', $request, json_encode($data), $user['id']);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
