...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'lookup-projects-list', $request, '', '');
...
{
	$result = Yii::$app->GeneralMod->getProjectList();
	$data['data'] = $result;
...
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'lookup-projects-list', $request, json_encode($data), 1);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
