...
$filePath = '/cmp/contents/associate/';
$request = array();
...
//create Log API
$logapi = Yii::$app->AccessMod->LogAPI('', 'member-upload-file', $request, '', '');
...
if(!strlen($data['error']))
{
	...
	if(!strlen($data['error']) and isset($_FILES['file']))
	{
		...
		else
		{
			...
			if(!move_uploaded_file($_FILES['file']['tmp_name'], $serverRoot.$filePath.$user['id'].'/'.$filename))
			$data['error'] = '0010-Uploading failed.';
			else
			$data['filepath'] = $http_host.$filePath.$user['id'].'/'.$filename;
		}
	}
}
...
if(!strlen($data['error']))
{
	$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'member-upload-file', $request, json_encode($data), $user['id']);
	if(isset($logapi['error']))
	$data['error'] = '0000-'.$logapi['error'];
}
