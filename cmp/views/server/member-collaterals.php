...
/*** create Log API ***/
$logapi = Yii::$app->AccessMod->LogAPI('', 'member-collaterals', $request, '', '');
...
if(!strlen($data['error']))
{
	$inputs['action'] = !empty($_REQUEST['action'])?strtolower($_REQUEST['action']):'collaterallist';
	$inputs['project'] = !empty($_REQUEST['project'])?$_REQUEST['project']:'';
	$inputs['collateral_id'] = !empty($_REQUEST['collateral_id'])?$_REQUEST['collateral_id']:'';
	$inputs['permalink'] = !empty($_REQUEST['permalink'])?$_REQUEST['permalink']:'';
}

//process
if(!strlen($data['error']))
{
	$result = Yii::$app->CatalogMod->getCollaterals($inputs);
...
if(!strlen($data['error']))
{
	$data['collaterals'] = $result;
}

//update log api
if(!strlen($data['error']))
{
$logapi = Yii::$app->AccessMod->LogAPI($logapi['logAPI']['id'], 'member-collaterals', $request, json_encode($data), $user['id']);
if(isset($logapi['error']))
$data['error'] = '0000-'.$logapi['error'];
}
