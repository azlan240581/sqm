...
<div class="prospects-form">
    ...
    <?= $form->field($modelPB, 'product_unit_price')->textInput(['maxlength' => true])
		->widget(NumberControl::classname(), [
			'maskedInputOptions' => [
    ...
    <?= $form->field($modelPB, 'eoi_ref_no')->textInput(['maxlength' => true,'disabled'=> 'disabled']) ?>
 
	<?php
	echo 	$form->field($modelPB, 'payment_method_eoi')->dropDownList(
			ArrayHelper::map($paymentMethods, 'id', 'name'),['prompt' => 'Please select','disabled'=> 'disabled']
			)->label($modelPB->getAttributeLabel('payment_method_eoi'));
    ?>

    <?= $form->field($modelPB, 'booking_eoi_amount')->textInput(['maxlength' => true,'disabled'=> 'disabled']) 
		->widget(NumberControl::classname(), [
			'maskedInputOptions' => [
				'prefix' => $_SESSION['settings']['CURRENCY_SYMBOL'].' ',
				'suffix' => '',
    ...
    <?= $form->field($modelPB, 'booking_date_eoi')->textInput(['maxlength' => true,'disabled'=> 'disabled']) ?>
