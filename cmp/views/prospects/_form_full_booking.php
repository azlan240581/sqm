...
$interProjects = Yii::$app->ProspectMod->getProspectInterestedProjects($model->id,false);
...
<div class="prospects-form">
    ...
    <?= $form->field($modelPB, 'product_unit_price', ['inputOptions' => ['class' => 'form-control', 'value' => !empty($modelPB->product_unit_price)?number_format($modelPB->product_unit_price,2,'.',''):'']])->textInput(['maxlength' => true]) ?>
    ...
    <?= $form->field($modelPB, 'eoi_ref_no')->textInput(['maxlength' => true]) ?>
    ...
    <?= $form->field($modelPB, 'booking_eoi_amount',['inputOptions' => ['class' => 'form-control', 'value' => !empty($modelPB->booking_eoi_amount)?number_format($modelPB->booking_eoi_amount,2,'.',''):'']])->textInput(['maxlength' => true]) ?>
    ...
    <?= $form->field($modelPB, 'booking_date_eoi')->textInput(['maxlength' => true]) ?>
    ...
    <?= $form->field($modelPB, 'booking_amount',['inputOptions' => ['class' => 'form-control', 'value' => !empty($modelPB->booking_amount)?number_format($modelPB->booking_amount,2,'.',''):'']])->textInput(['maxlength' => true]) ?>
	...
	echo $form->field($model, 'prospect_identity_document')->widget(FileInput::classname(), [
		...
		'pluginOptions' => [
				...
				//'required'=> true,
				'initialPreview' => [
					$model->prospect_identity_document, // checks the models to display the preview
				],
				'initialPreviewConfig' => [
								[
    ...
    <div class="form-group">
        <?= Html::submitButton('Booking', ['class' => 'btn btn-success']) ?>
    </div>
...
{
	$.ajax({url: "/cmp/prospects/eoi-booking?id="+<?= $model->id ?>+"&project_id="+projectid, dataType: "json", success: function(result){
...
{
	$.ajax({url: "/cmp/prospects/eoi-booking?id="+<?= $model->id ?>+"&product_id="+productid, dataType: "json", success: function(result){
		//alert(JSON.stringify(result));
